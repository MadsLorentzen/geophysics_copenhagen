function YPrime = geqDEMYPrime(t,y)
%% function YPrime = geqDEMYPrime(t,y)
% used in geqDEM
% 
...
k1=DEMINPT.K1; 
mu1=DEMINPT.MU1; 
nrOfInclusion=numel(DEMINPT.ASP);
% inclusionIndex = 1;
% while (t > sum(DEMINPT.Concentration(1:inclusionIndex)))
%     inclusionIndex = inclusionIndex + 1;
% end
krhs = 0;
murhs = 0;
for inclusionIndex = 1 : nrOfInclusion
    k2=DEMINPT.K(inclusionIndex); mu2=DEMINPT.MU(inclusionIndex);
    asp=DEMINPT.ASP(inclusionIndex); phic=DEMINPT.Phic;
    conc=DEMINPT.Concentration(inclusionIndex)/sum(DEMINPT.Concentration);
...
%         % Disks
%         if (asp < 0.1)
%             zeta    = mua/6*(9*ka+8*mua)/(ka+2*mua);
%             pa      = (k+4/3*mua)./(ka+4/3*mua); 
%             qa      = (mu + zeta)/(mua + zeta);
%         end
%         %%
    ...
    krhs=krhs+conc*(ka-k)*pa;
%     yprime(1)=krhs/(1-t);

    murhs=murhs+conc*(mua-mu)*qa;
%     yprime(2)=murhs/(1-t);
end
